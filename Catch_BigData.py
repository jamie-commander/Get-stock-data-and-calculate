...
class Stock_Data():
    def __init__(self,data="123"):
        self.data = data
        True
    def get(self,data_name=["Close"],days=1,before = 0 ,pieces = 1):
        
        flag = True
        for i in data_name:
            if((i in self.data) == False):
                flag = False
        if(flag):
            if(days>0):
                pd_data = self.data[data_name][-days+1-pieces:] if before==0 else self.data[data_name][-days-before+1-pieces:-before]
                #return_data = np.resize(return_data[len(return_data)%pieces:],(int(len(return_data)/pieces),pieces*len(data_name)))
                return_data = np.zeros(shape=(len(pd_data)- pieces + 1,len(data_name)*pieces))
                #return_data =np.reshape(return_data,(-1,20))
                #print(pd_data[data_name][0:0+pieces])
                #print(np.resize(np.array(pd_data[data_name][0:0+pieces]), (1, len(data_name)*pieces)).flatten())
                #print(pd_data)
                for i in range(0,len(pd_data) - pieces + 1):
                    #print(pd_data[data_name][i:i+pieces])
                    s = np.reshape(np.array(pd_data[data_name][i:i+pieces]), (1, len(data_name)*pieces)).flatten()
                    return_data[i] = np.array(s)
                return return_data
            else:
                return np.nan
        else:
...
class Catch_Stocks_BigData():
    ...
    def ALL_Data_main(self):#上櫃市場日期格式 民國/月/日 上市市場日期格式 西元月日
        ...
        self.path_data_url_Stocks_list = {
            'tse' : {self.path_data['tse']['Stocks'] : ['https://www.twse.com.tw/exchangeReport/MI_INDEX?response=json&date=','&type=ALLBUT0999'],#網址+日期
                    },
